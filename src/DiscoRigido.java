...
public class DiscoRigido {
	...
	private static void adicionarDiretorioAoBloco(Diretorio diretorio) {
		for (int i = 0; i < 4; i++) {
			if (blocos[i].espacoRestante >= 10) {
				...
				blocos[i].setEspacoRestante(blocos[i].getEspacoRestante() - 1);
				tamanhoDoDiscoDisponivel -= 1;
				System.out.println(
						"DIRETÓRIO CRIADO NOME: " + diretorio.getNome() + " INCLUIDO NA RAIZ, " + blocos[i].getNome());
				imprimeEspacoDisponivelNoDisco();
				break;
			}
		}
			...
			if (blocos[1].getListaDeDiretorios() != null) {
				for (Diretorio d : blocos[i].getListaDeDiretorios()) {
					if (d.getNome().equals(arquivo.getNomeDiretorio())) {
						...
						System.out.println("ARQUIVO: " + arquivo.getNome() + ", ADICIONADO COM SUCESSO NO DIRETÓRIO: "
								+ d.getNome() + ", LOCALIZADO NA RAIZ, " + blocos[i].getNome());
						imprimeEspacoDisponivelNoDisco();
						return true;
					}
				}
	...
	public void listarDiretoriosEArquivos() {
